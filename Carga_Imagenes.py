...
if img is not None:
    ...
    if sync:
        ...
        idx = st.sidebar.slider('Corte (sincronizado)', 0, lims[corte], mids[corte])
        slice_idx = st.sidebar.number_input('Corte (sincronizado)', 0, lims[corte], idx)
    else:
        corte = st.sidebar.radio('Selecciona corte',('Axial','Coronal','Sagital'))
    ...
    invert = st.sidebar.checkbox('Invertir colores', False)
    ...
    for name,mat in [('Axial',axial),('Coronal',coronal),('Sagital',sagital)]:
        st.markdown(name)
    ...
    # 3D con agujas
    show3d=st.sidebar.checkbox('Vista 3D', True)
    if show3d:
        ...
        with st.expander('Agregar aguja 3D'):
            ...
            if st.button('Manual Recta'):
                st.session_state['needles'].append({'points':((x1,y1,z1),(x2,y2,z2)),'color':f"#{random.randint(0,0xFFFFFF):06x}",'curved':False})
            if st.button('Manual Curva'):
                st.session_state['needles'].append({'points':((x1,y1,z1),(x2,y2,z2)),'color':f"#{random.randint(0,0xFFFFFF):06x}",'curved':True})
            if st.button('Aleatoria Recta'):
            ...
            if st.button('Aleatoria Curva'):
                xa,ya,za=[random.uniform(7,35) for _ in range(3)]
                xb,yb,zb=[random.uniform(30,45) for _ in range(3)]
                st.session_state['needles'].append({'points':((xa,ya,za),(xb,yb,zb)),'color':f"#{random.randint(0,0xFFFFFF):06x}",'curved':True})

        # Tabla de registro con tipo
        st.markdown('### Registro de agujas')
        df=pd.DataFrame([{
            'ID':i+1,'X1':round(p[0][0],1),'Y1':round(p[0][1],1),'Z1':round(p[0][2],1),
        ...
        fig3d=go.Figure(data=[go.Volume(x=xg.flatten(),y=yg.flatten(),z=zg.flatten(),value=resized.flatten(),opacity=0.1,surface_count=15,colorscale='Gray')])
